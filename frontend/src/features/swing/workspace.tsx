    ...
    <div className="grid gap-5 xl:grid-cols-[minmax(18rem,0.8fr)_minmax(0,1.4fr)_minmax(16rem,0.7fr)]">
      ...
      <section className="min-w-0">
        ...
        <div className="mt-3 overflow-x-auto border-t border-line">
          <table className="w-full min-w-[36rem] text-left text-xs"><caption className="sr-only">Dated OHLCV and backend indicator values</caption><thead><tr>{["Session", "Status", "Open", "High", "Low", "Close", "Volume", "EMA20", "EMA50", "RSI14", "ATR14", "Traded value"].map((label) => <th className="px-2 py-2 font-semibold" key={label} scope="col">{label}</th>)}</tr></thead><tbody>{chart.map((point) => <tr key={point.session_date} className="border-t border-line"><th scope="row" className="px-2 py-2">{point.session_date}</th><td className="px-2 py-2">{point.status.replaceAll("_", " ")}</td>{[point.open, point.high, point.low, point.close].map((value, index) => <td className="px-2 py-2" key={index}>{value?.amount ?? "—"}</td>)}<td className="px-2 py-2">{point.volume ?? "—"}</td>{["ema20", "ema50", "rsi14", "atr14", "traded_value20"].map((key) => { const metric = point.indicators[key]; return <td className="px-2 py-2" key={key}>{metric?.value ?? "—"}{metric?.basis === "estimated" ? " est." : ""}</td>; })}</tr>)}</tbody></table>
