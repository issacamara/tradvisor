...
`annual_financial_revisions_v1` is the owner-selected additive, versioned canonical annual-financial contract. It preserves the legacy `financials` table and keys a revision by company, fiscal period, report scope, source, source observation, and `revision_id`. The financial fields mirror the normalized V1 annual contract: currency and original scale, revenue, ordinary-owner earnings, equity and opening equity, interest-bearing debt, unrestricted cash, current assets, current liabilities, publication status, and reason codes, plus source/revision metadata. The four additional `NUMERIC NULLABLE` amounts apply to non-financial operating-company inputs and require source evidence with coherent issuer, reporting scope, period, units, and currency. Unsupported, unreported, restricted, or otherwise unverified values remain `NULL`; null is not zero, and reason-code semantics remain unchanged. The contract is versioned in the table name so a future incompatible shape can be introduced without silently reshaping existing rows.
