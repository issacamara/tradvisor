...
- Normalized prices carry `close_basis` as `raw`, `adjusted`, or `unknown` (legacy/missing evidence is `unknown`). Estimated turnover requires `close_basis=raw` and compatible unadjusted share volume; adjusted or unknown close basis is unavailable. Current-session eligibility additionally requires the expected session date, an actual traded row and raw actual close with actual provenance and validated availability evidence.
