...
- The ingestion baseline was refreshed from production commit `3c185f5`. The scripts were copied exactly, and the working branch is V1. Architecture improvements below are proposed implementation work, not changes already applied to those scripts.
...
| BRD-A-05 | Architecture discussion: V1 development alignment | Target dev-tradvisor; preserve development ownership and data, use production as read-only functional baseline, review plans before deployment | Ingestion, Analytical Data, Pipeline Orchestration, Investor Application, Application Store | Reused / New |
...
The stakeholder delegated remaining financial-rule decisions on 2026-09-19. [Financial Calculation Contract v1.1](tradvisor_financial_contract.md), sections 2 and 6, is normative for Swing initialization, session modeling, warm-up, liquidity basis, current-trade eligibility and factual evidence labels. Source availability and empirical evaluation remain implementation/release dependencies, not unresolved strategy choices.
...
- New: no inspected implementation supplies the V1 transactional paper ledger. Existing legacy `trading_dashboard.users` tables and development web resources are not this ledger and must be preserved; no identity/data migration is implied.
...
|----------------|----------|-------------|
| `terraform/main.tf` | Google Cloud provider and BigQuery dataset foundation | Existing platform matches the BRD constraint |
| `terraform/functions.tf` | Scheduler and workflow patterns | Already models scheduled ingestion and authenticated calls |
| `terraform/buckets.tf` | Source and archive storage foundation | Existing storage pattern can preserve source evidence |
| `terraform/iam.tf` | Deployment identity and access structure | Can be adapted to workload identities and narrower permissions |
| `archive/legacy-ingestion/scripts/helper.py` | Adapt storage and BigQuery load functions | Implements the selected source-to-storage integration; needs explicit schemas and retry-safe loading |
| `archive/legacy-ingestion/scripts/scrape_shares.py` and `insert_shares.py` | Share extraction and loading | Confirmed source code contains the core OHLC and volume extraction |
| `archive/legacy-ingestion/scripts/scrape_dividends.py` and `insert_dividends.py` | Dividend extraction and loading | Confirmed source code extracts dividend amounts and payment dates |
| `archive/legacy-ingestion/scripts/scrape_financials.py`, `scrape_financials_init.py`, and `insert_financials.py` | Financial report discovery, PDF acquisition, model extraction, and BigQuery upserts | Production code implements the required financial fields and historical initialization; execution paths need alignment |
| `archive/legacy-ingestion/scripts/scrape_ratings.py`, `scrape_ratings_init.py`, and `insert_ratings.py` | Rating discovery, history collection, and loading | Production code extracts short-term and long-term ratings; the loader contract and history policy need correction |
| `get_companies_from_brvm()` in `scrape_financials_init.py` and `mapping.csv` | Company discovery and issuer-to-symbol mapping | Existing code can seed reference identity, but lacks sector/activity enrichment and a company-table load |
| `.github/workflows/ci.yml` | Extend existing validation | Already checks Python syntax and Terraform; adapter fixture and repeatability tests will be added |

### Confirmed Source Coverage

| BRD source | Archived implementation and configured origin | Reuse decision / gap |
|------------|-------------------------------------------------|---------------------|
| `shares` | `scrape_shares.py` and `insert_shares.py`; configured Sikafinance market page | Reuse extraction and load path. Preserve decimal values and obtain an authoritative session date. |
| `dividends` | `scrape_dividends.py` and `insert_dividends.py`; Richbourse embedded JSON with HTML fallback | Reuse extraction and MERGE path. Fiscal year is present but may be inferred from payment year. Normalize producer/loader field names and preserve distinct distributions. |
| `brvm_companies` | BRVM company-list discovery inside `scrape_financials_init.py`, plus `mapping.csv`; no company-table loader found | Reuse discovery and symbol mapping. Add explicit reference-table loading and obtain sector/activity metadata; existing live-table provenance is unverified. |
| `financials` | Richbourse announcement processing, BRVM historical PDF discovery, OpenRouter extraction, and BigQuery upserts | Reuse all stages. Align initialization/monthly interfaces, preserve evidence, and consolidate duplicated extraction/upsert logic. |
| `ratings` | Richbourse per-symbol collection, initialization, and file loading | Reuse scrapers; adapt the loader to rating-specific fields and keys, preserve dated history, and document rating-scale interpretation. |

The adapter findings describe inspected code, not verified current website behavior or live pipeline success. The later environment assessment inspected BigQuery metadata and selected schemas, not historical rows; calendar sources were separately reviewed in the integration contract. No production coverage certification is implied.

Archived bonds, indices, and capitalizations pipelines remain reference assets. Their presence does not add those product features to V1. The company roster discovered from BRVM reports and the mapping file remains incomplete until sector and activity metadata are supplied.

### Planned Ingestion Improvements

| Evidence in archived code | Proposed implementation change | Acceptance evidence |
|---------------------------|--------------------------------|---------------------|
| `scrape_shares.py` sets `date` to the collection day | Record `collected_at` separately from the authoritative trading-session date; never relabel an unchanged prior-session quote as a new session | Recollection on a non-trading day preserves the original session |
| Share parser removes every non-digit character before numeric conversion | Parse numeric fields with explicit source-locale rules, preserving decimal separators and distinguishing missing values | Stored source fixtures retain exact decimal prices and represent missing values explicitly |
| Shares parse responses without a status check; production dividends already check status and neither scraper explicitly disables certificate verification | Preserve current certificate verification and dividend checks; add consistent error handling and bounded retries to remaining paths | Failed responses cannot publish successful ingestion runs |
| Raw CSV names contain only asset and collection date | Persist immutable raw responses and normalized extracts under source/run identifiers, with content hashes and parser versions | Repeated same-day collections preserve distinct evidence and can be replayed |
| Shares append before archiving; the financial/dividend MERGE helper uses a shared `table_temp` staging table | Reuse MERGE with explicit source keys and per-run staging; serialize publication per dataset | Retried and overlapping runs do not duplicate records or consume another run's staging data |
| Dividend scraper emits uppercase fields while the loader expects lowercase keys; fiscal year can be guessed and MERGE uses only symbol/year | Normalize schema explicitly, distinguish inferred years, and preserve distribution identity before annual aggregation | Producer/loader contract tests pass; separate installments and cross-year payments remain correctly represented |
| Ratings output omits `date`, but the generic loader unconditionally reads `df['date']` | Give ratings an explicit schema, collection timestamp, and deduplicated load contract | A rating batch loads without a price-date field and a repeat load produces no duplicate observations |
| Ratings and dividends delete older rows using current calendar-year cutoffs | Preserve raw/revision history and apply analysis windows in derived datasets; choose canonical retention explicitly | Historical results can still be reconstructed after a new year starts |
| Monthly financial scraper calls its initialization function with two arguments, while the initialization function accepts one | Align initialization and incremental entry points and document which stage loads PDFs versus extracted rows | Empty-table initialization and incremental-update fixtures both complete the intended stages |
| Financial extraction uses model calls and has duplicate logic across two files | Share the extraction adapter; persist source hash, model/prompt version, response artifact, and normalized result | Analysis replay uses stored extraction results and makes no external model call |
| Some local imports still execute ingestion entry points | Use explicit executable entry points while preserving existing guarded modules | Importing modules for tests performs no network or storage writes |
| Workflow pairing relies on list positions and a fixed offset | Define explicit extractor/loader dependencies per source | Adding a source does not change the pairing of existing pipelines |
| V1 Terraform packages only main/helper/config/requirements and does not register financials or ratings jobs | Include required initialization modules and mapping resources; register explicit workflows and credential injection | Deployment artifacts include every imported local module and the intended schedules cover all V1 sources |

These changes belong to implementation and automated tests; they do not create a separate data-quality user workflow.

## 7. Data Architecture

[Source-To-Calculation Mapping v0.5](tradvisor_source_mapping.md) maps the five supplied schemas and two read-only CSV samples to Swing, paper, Growth and dividend-research dependencies. It distinguishes field presence from verified semantics/coverage, identifies derivations and missing inputs, and provides MAP-01 through MAP-10 adapter work with two grouped source-confirmation questions. The source-mapping pass did not query production rows; later environment checks inspected metadata and selected schemas; a targeted scraper read confirmed runtime-date assignment, distinct from the stakeholder's intended trading-date meaning. This is a reviewed-document integration aid, not certification of production coverage or a change to the financial rules.
...
- Deployment: build on the inspected Terraform foundation. `V1` targets `dev-tradvisor`, using `prod-tradvisor` as the production baseline plus the approved V1 additions. Keep environment-specific identities, configuration, data and Terraform state isolated; production promotion requires separate approval. Preserve existing development resources unless an explicit migration/removal is approved. The [Development Environment Alignment assessment](tradvisor_environment_assessment.md) records the 2026-09-20 read-only live comparison, observed drift, proposed reuse/update/add matrix and remaining verification. Repository configuration alone is not proof of live resource ownership or parity. New development schedules should remain paused until dependency and cost checks pass and activation is approved; do not clone production users, secrets or backfills.
...
Deployment preparation update, 2026-09-20: the [Infrastructure Change Plan](tradvisor_infrastructure_change_plan.md) requires preserving development-owned legacy resources before any Terraform apply. Backend prefixes and state addresses are now evidenced, and selected workflow dependencies and shares schema differences are verified. This changes deployment sequencing, not the approved application components or financial rules. Production state contains at least one service not found by the live API; neither production state nor repository configuration is a complete source of live truth. No deployment is authorized by this update.
...
| Existing development state owns resources omitted from V1 Terraform | Unintended deletion, key rotation or IAM loss on apply | Confirmed ownership/configuration mismatch | Preserve declarations/addresses first; serialize infrastructure work and reject unsafe full plans before any apply |
| Production references or stale state copied into development | Cross-project access or unintended resource creation | Observed state/live drift; identity trust not fully checked | Rebuild development references, preserve separate backends and verify live ownership; no wholesale state/config clone |
...
**Document validation (2026-09-20):** Checked all 34 numbered BRD functional/nonfunctional/data requirement IDs against section 3, all 14 sections, seven component classifications, nine active assumptions, handoff YAML, local document links and Markdown fences. Reviewed the approved scope amendments and integration precedence, including deferred Dividend/Balanced scoring and restrictive-intent-first reset. This is document-level validation only; no application tests, Terraform plan, cloud mutation or GitHub backlog creation occurred.

Use the following as work packages, not atomic tickets. Split each into single-responsibility testable tasks (especially L packages), with architecture/requirement links, interfaces, acceptance tests, hard dependencies and overlapping-file exclusions. Do not dispatch agents or create GitHub issues as part of this document-finalization step. The orchestrator must read the full normative package and verify the target repository before publishing its tracking epic and tickets.
...
| ORCH-02 | Preserve development Terraform ownership and explicit backend/project targeting | Recheck filtered ownership and current live metadata under infrastructure change set 0 | M; retain missing legacy declarations and addresses, current schedules and IAM members; no unapproved key rotation |
...
| ORCH-05 | Adapt ingestion packaging, per-function settings and explicit workflows | ORCH-02/03 for infrastructure changes; ORCH-04 source-specific contracts | L; reuse financial PDF code; retry-safe snapshots/loads, isolated development endpoints, preserved legacy indices and new schedules paused |
| ORCH-06 | Add lowercase source-table and V1 platform configuration | ORCH-02/03 and verified relevant schemas/locations | L; preserve uppercase tables and legacy users, least-privilege new identities, static hosting/auth, scale-to-zero API/jobs, store/recovery register; reviewed plan and explicit approval before provisioning |
...
**Deployment gate:** Follow the [Infrastructure Change Plan](tradvisor_infrastructure_change_plan.md). Current development backend is `gs://dev-tradvisor-tfstate/tradvisor/V1/default.tfstate`; production baseline is read-only. Recheck current ownership and serials rather than treating the 2026-09-20 snapshot as a lock. Preserve development legacy service, secrets, identities, tables and schedules. Do not copy production state, secrets or users, shift existing workflow indices, or recreate stale production resources. Each concrete plan/apply and any schedule activation/data seeding requires its own approval. A failing preservation/isolation check blocks affected deployment tickets, not unrelated fixture-based implementation.
