...
def load_files(config):
    ...
    if config['environment'] == 'gcp':
        ...
        return bucket.list_blobs(prefix='CAPITALIZATIONS')

    else:
        return glob.glob(os.path.join(os.path.join(os.path.dirname(__file__), '', config['csv_directory']), "CAPITALIZATIONS*.csv"))
...
def process_csv_files(files, conf):
    if conf['environment'] == "gcp":
        for f in files:
            ...
            insert_into_bigquery(df, project_id, conf['gcp']['bigquery']['dataset'], 'CAPITALIZATIONS')

    else:
        for f in files:
            df = pd.read_csv(f, sep='|')
            insert_into_duckdb(df, conf['duckdb']['database'], 'CAPITALIZATIONS')
